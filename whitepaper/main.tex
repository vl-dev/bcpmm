...
The right-hand side of \cref{eq:profitability-M} satisfies:
\begin{equation}
    (A+V)B(B-x-y) \ge (A+V)B \cdot 0.01B = 0.01B^2 (A+V).
\end{equation}
The left-hand side is bounded by $(T-y)(B-x) < T B$. For profitability, we would need $T B > 0.01B^2 (A+V)$, or equivalently $T > 0.01B (A+V)$.

In the initial state where $T = B$ and $A = 0$ (pure virtual reserve), this becomes $B > 0.01B V$, i.e., $1 > 0.01V$, which may hold for small $V$. However, this uses the worst-case bound on $B - x - y$. For typical attack scenarios with moderate $x$ (e.g., $x < 0.5B$), we have $B - x - y > 0.49B$, making the right-hand side at least $0.49B^2 (A+V)$, which requires $T > 0.49B (A+V)$.

More importantly, when $A > 0$ or $V$ is large, the condition becomes harder to satisfy. The top-up mechanism with $N \le M$ is designed to preserve the original price profile, and the virtual reserve adjustment $V_2$ from \cref{eq:V2-with-topup} ensures solvency while limiting extractable value.

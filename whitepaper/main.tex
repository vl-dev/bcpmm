...
However, this does not protect against Sibyl attacks and is mainly implemented as a convenience mechanism to be able to track the faithful users and restrict their amount of impactful off-chain activity.
...
To make the system robust in practice, we propose predictable fixed-percentage burns (a single configured fraction of the current base reserve, bounded by the fee rate), an optional Burn Authority ensuring that burns are tied to off-chain actions, time-based throttling of burn frequency, and per-wallet daily burn limits via a lightweight on-chain accounts (optionally generalized via compressed state). These mechanisms reduce the probability that the worst-case top-up scenario can be orchestrated repeatedly, and they remain compatible with fully decentralized configurations when central authority is disabled.
